...
class Admin extends PluginBase
{
    ...
    {   
        ...
        add_action('admin_action_chatwing_float_ui_save_token', array($this, 'handleTokenSaving'));
        add_action('admin_action_chatwing_float_ui_save_settings', array($this, 'handleSettingsSave'));
    }
    ...
    {
      add_menu_page(__('Chatwing Float UI plugin settings', CHATWING_FLOAT_UI_TEXTDOMAIN), 'Float Chatbox', 'manage_options', 'chatwing_float_ui', array($this, 'showSettingsPage'));
    }
    ...
    public function handleTokenSaving($skipNonce = false)
    {
        ...
        wp_redirect('admin.php?page=chatwing_float_ui');
        die;
    }
    ...
    public function handleSettingsSave()
    {
        ...
        foreach($fieldsToUpdate as $field) {
          if (!empty($_POST[$field]) && is_numeric($_POST[$field])) {
            update_option('chatwing_float_ui_default_' . $field, $_POST[$field]);
          }
        }
    ...
    public function loadTemplate($templateName, $data = array())
    {
      if (strpos($templateName, '.php') === false) {
        ...
        $file = CHATWING_FLOAT_UI_TPL_PATH . '/' . $templateName;
        if (file_exists($file)) {
          ob_start();
          if (!empty($data)) {
            extract($data);

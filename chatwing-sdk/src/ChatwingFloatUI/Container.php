...
class Container implements ArrayAccess
{
    ...
    public function bind($key, $value)
    {
        if ($this->isFrozen($key)) {
            throw new \Exception( __("Can not overwrite the element with key '{$key}' !! ", CHATWING_FLOAT_UI_TEXTDOMAIN));
        }
     ...
     * @param string $key
     * @param mixed $value
     * @return \ChatwingFloatUI\Container
    ...
    public function factory($key, $callable)
    {
        ...
        if (!(is_object($callable) && method_exists($callable, '__invoke'))) {
            throw new \Exception(__("Value should be a Closure or object/class with __invoke method!!", CHATWING_FLOAT_UI_TEXTDOMAIN));
        }
